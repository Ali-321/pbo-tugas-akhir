 ...
 * @author Lenovo
 */
public class MKurangBarang {
   static int id;

    
    public MKurangBarang(){
            try{
              String call = "{CALL get_end_iddj()}";
     ...
     public void showTable(JTable tblBrg ,String kodeJual){
    
        
            try{
              String call = "{CALL get_transaksi_bk2(?)}";
                 CallableStatement cstmt = (CallableStatement)Conn.connect().prepareCall(call);
     ...
     public void update(int id,String kodeJual,String kodeBrg,int jml){
         
            try{
             
                 String call = "{CALL update_djual(?,?,?,?)}"; 
                 CallableStatement cstmt = (CallableStatement)Conn.connect().prepareCall(call);
                 
                 cstmt.setInt(1, id);
                 cstmt.setString(2, kodeJual);
